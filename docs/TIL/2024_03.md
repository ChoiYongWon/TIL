...
4. [Browser] js 번들을 다시 서버에 요청하여 ClientComponent를 Hydrate한다.  
5. [Browser] 이미 렌더링 된 ServerComponent에 대한 렌더트리 정보는 js 번들에 포함되어있지 않으므로 inline script에 포함되어있는 ServerComponent를 직렬화한 결과를 해석하여 가상돔에 포함시킨다.  
