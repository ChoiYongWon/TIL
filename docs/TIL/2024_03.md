...
[NextJS 공식 문서 Interleaving Server and Client Components](https://nextjs.org/docs/app/building-your-application/rendering/composition-patterns#interleaving-server-and-client-components)
...
NextJS에선 다음과 같이 말하고 있다.

> Provider를 트리에서 가능한 한 깊숙이 렌더링해야 합니다. ThemeProvider가 전체 `<html>` 문서 대신 `{children}`만 래핑하는 방식을 주목하세요. 이렇게 하면 Next.js가 서버 컴포넌트의 정적 부분을 더 쉽게 최적화할 수 있습니다.

- 이렇게 하면 추가적으로 Client Javascript Bundle Size도 줄일 수 있다고 한다.

- 서버 컴포넌트에서 데이터를 가져오는 경우, 클라이언트 컴포넌트에 프롭으로 데이터를 전달하고 싶을 수 있을 수 있는데, 서버에서 클라이언트 컴포넌트로 전달되는 프롭은 React에서 직렬화할 수 있어야 한다.
...
5. 이미 렌더링 된 ServerComponent는 js 번들에 포함되어있지 않으므로 inline script에 포함되어있는 ServerComponent를 직렬화한 결과를 해석하여 가상돔에 포함시킨다.
