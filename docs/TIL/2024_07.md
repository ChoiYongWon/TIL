...
브라우저는 HTML을 전부 해석하기 전까진 페인팅을 안해준다.  

즉, BuildTime CSS에서는 link 태그에 있는 CSS를 요청하고 CSSOM -> Render Tree등으로 처리하는 과정으로 진행 되는 반면, RunTime CSS에서는 JS를 해석하는 과정 + CSSOM을 다시 계산하여 Render Tree에 반영하는 시간도 있고 JS에서 Runtime으로 CSS를 생성할 때 렌더링 엔진이 JS 엔진에게 제어권을 위임하기 때문에 렌더링 과정이 수행되지 않기에 더 오래 걸린다는 차이점이 있다. 

정리하자면 BuildTime Css와 RunTime Css 모두 페인팅하기 전에 CSS가 반영되지만 속도면에서 차이가 있다.  
